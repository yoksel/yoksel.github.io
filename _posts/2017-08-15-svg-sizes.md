...
title: Размеры в SVG
type: post
image: '//img-fotki.yandex.ru/get/227342/5091629.a5/0_93a14_d716e495_orig'
...
Отсчет системы координат вьюпорта начинается от левого верхнего угла вьюпорта, системы координат SVG-элемента — от левого верхнего края вьюбокса.

По умолчанию система координат SVG-элемента соответствует системе координат вьюпорта, а единицы измерения SVG-элемента — единицам измерения вьюпорта, но при использовании трансформаций или <code>viewBox</code> масштабируется вся система координат с единицами измерения, то есть пиксели из <i>user space</i> больше не равны пикселям из <i>viewport space</i>.

Поизменяйте размеры элемента и посмотрите что происходит с системой координат содержимого (она показана бирюзовым):

<p data-height="500" data-theme-id="4974" data-slug-hash="VzWXZz" data-default-tab="result" data-user="yoksel" data-embed-version="2" data-pen-title="SVG with & coordinats" class="codepen">See the Pen <a href="https://codepen.io/yoksel/pen/VzWXZz/">SVG with & coordinats</a> by yoksel (<a href="https://codepen.io/yoksel">@yoksel</a>) on <a href="https://codepen.io">CodePen</a>.</p>
<script async src="https://production-assets.codepen.io/assets/embed/ei.js"></script>
...
Если такое поведение обводки нежелательно, это можно исправить с помощью свойства `vector-effect` со значением `non-scaling-stroke`, оно добавляется к содержимому SVG:
